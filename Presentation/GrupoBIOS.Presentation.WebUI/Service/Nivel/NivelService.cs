...
namespace GrupoBIOS.Presentation.WebUI.Service.Nivel
{
    public class NivelService : INivelService
    {
        ...
        public NivelService(HttpClient httpClient)
        {
            _httpClient = httpClient;
        }
        public async Task<string> Register(List<NivelDTO> register)
        {
            Response<string>? request = new Response<string>();

            var registerAsJson = JsonSerializer.Serialize(register);

            var response = await _httpClient.PostAsync("api/Nivel/CrearActualizarNivelesAsync",
