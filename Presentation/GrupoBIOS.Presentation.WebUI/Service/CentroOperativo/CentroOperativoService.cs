...
namespace GrupoBIOS.Presentation.WebUI.Service.CentroOperativo
{
    public class CentroOperativoService : ICentroOperativoService
    {
        ...
        public CentroOperativoService(HttpClient httpClient)
        {
            _httpClient = httpClient;
        }
        public async Task<string> Register(List<CentroOperativoDTO> register)
        {
            Response<string>? request = new Response<string>();

            var registerAsJson = JsonSerializer.Serialize(register);

            var response = await _httpClient.PostAsync("api/CentroOperativo/CrearActualizarCentroOperativoAsync",
